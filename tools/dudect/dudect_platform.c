...
void
ichor_dudect_check_governor(FILE *out)
{
 ...
 * --reps is not specified.  The intent is to ensure per-trial wall
 * time stays well above the timer's granularity.
 *
 ...
 * existing reps values produce per-trial durations comfortably above
 * the 10 us design target on both x86_64 and Apple Silicon, so
 ...
 */
int
ichor_dudect_default_reps_scale(void)
{
    return 1;
}
