 ...
 *
 * dudect harness entry point.  Empirical constant-time validation of
 * ichor's software primitives via Welch's t-test over two input
 * classes; see README.md for methodology and acceptance criteria.
 */
...
extern const dudect_target_t target_clmul64_scalar_b;
extern const dudect_target_t target_clmul64_scalar_a;
           ...
           int reps_override, double crop_lo, double crop_hi, int verbose)
{
    ...
    if (reps_override > 0) {
        ...
        int scale = ichor_dudect_default_reps_scale();
    ...
    for (int w = 0; w < 1000 && !g_stop; w++) {
        ...
        (void)ichor_dudect_now_ticks();
        for (int i = 0; i < reps; i++)
            t->run(state);
        (void)ichor_dudect_now_ticks();
    }

    fprintf(stdout, "ichor-dudect: %s (reps=%d, target samples/class=%zu)\n",
    ...
    for (size_t trial = 0; trial < total_trials && !g_stop; trial++) {
        ...
        uint64_t t0 = ichor_dudect_now_ticks();
        for (int i = 0; i < reps; i++)
            t->run(state);
        uint64_t t1 = ichor_dudect_now_ticks();
...
main(int argc, char **argv)
{
    ...
    printf("ichor-dudect timing-validation harness\n");
    printf("  timer:   %s\n", ichor_dudect_timer_name());
    printf("  crop:    [%.2f%%, %.2f%%]\n", crop_lo, crop_hi);
    printf("  samples: %zu per class\n", samples);

    if (do_pin) {
        int pr = ichor_dudect_pin_cpu(pin_cpu);
        if (pr == 0) {
    ...
    ichor_dudect_check_governor(stdout);
    printf("\n");
