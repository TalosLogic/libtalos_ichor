 ...
 *
 * Compiled only when ICHOR_HAVE_AES_NI is defined.  This TU is
 * always listed in ICHOR_CORE_SOURCES but its entire content is
...
static void key_expand_128_ni(ichor_aes_ctx_t *, const uint8_t *);
static void key_expand_192_ni(ichor_aes_ctx_t *, const uint8_t *);
static void key_expand_256_ni(ichor_aes_ctx_t *, const uint8_t *);
static void encrypt_ni(const ichor_aes_ctx_t *, uint8_t[16], const uint8_t[16]);
...
static void
key_expand_128_ni(ichor_aes_ctx_t *ctx, const uint8_t *key)
{
...
static void
key_expand_192_ni(ichor_aes_ctx_t *ctx, const uint8_t *key)
{
    /*
    ...
    /*
     * Load the trailing 8 key bytes with _mm_loadl_epi64 (low 64 bits, high
     * half zeroed): the unaligned may-alias intrinsic idiom, matching the
     * _mm_loadu_si128 loads on the 128/256 paths.  A raw
     * *(const uint64_t *)(key + 16) would be a strict-aliasing violation and
     * assume an alignment the caller does not guarantee.
     */
...
static void
key_expand_256_ni(ichor_aes_ctx_t *ctx, const uint8_t *key)
{
    __m128i *rk = (__m128i *)ctx->storage;
    __m128i k1 = _mm_loadu_si128((const __m128i *)key);
...
static void
encrypt_ni(const ichor_aes_ctx_t *ctx, uint8_t out[16], const uint8_t in[16])
{
...
int
aes_aesni_key_expand(ichor_aes_ctx_t *ctx, const uint8_t *key, int key_bits)
{
    int nr;
    ...
    ctx->backend_tag = ICHOR_AES_BACKEND_AESNI;
    return 0;
}

void
aes_aesni_encrypt(const ichor_aes_ctx_t *ctx, uint8_t out[16],
                  const uint8_t in[16])
{
 ...
 * single-block calls.  The uniform ichor_aes_encrypt_x4 API allows
 * the PRG to amortize its per-call overhead uniformly across backends.
 */
void
aes_aesni_encrypt_x4(const ichor_aes_ctx_t *ctx, uint8_t out[64],
                     const uint8_t in[64])
{
...
const ichor_aes_ops_t ichor_aes_ops_aesni = {
    .key_expand = aes_aesni_key_expand,
    .encrypt = aes_aesni_encrypt,
    .encrypt_x4 = aes_aesni_encrypt_x4,
    .backend_tag = ICHOR_AES_BACKEND_AESNI,
    .name = "aesni",
};

#else /* !ICHOR_HAVE_AES_NI */
...
typedef int ichor_aes_aesni_translation_unit_not_empty;
