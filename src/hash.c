...
static void
sponge_init(ichor_hash_ctx_t *ctx, size_t rate, uint8_t suffix)
{
...
static int
sponge_absorb(ichor_hash_ctx_t *ctx, const uint8_t *data, size_t len)
{
    /*
     * Contract (hash.h): once squeeze/finalize has run, no further absorb is
     * allowed.  Absorbing into a squeezing sponge would silently mutate the
     * output state and yield a wrong hash.  Refuse the misuse: leave the state
     * untouched and return an error.
     */
    if (ctx->finalized)
        return ICHOR_HASH_ERR_FINALIZED;
...
static void
sponge_finalize(ichor_hash_ctx_t *ctx)
{
    if (ctx->finalized)
        return;
...
static void
sponge_squeeze(ichor_hash_ctx_t *ctx, uint8_t *out, size_t len)
{
    if (!ctx->finalized)
        sponge_finalize(ctx);
...
void
ichor_sha3_256_init(ichor_hash_ctx_t *ctx)
{
    sponge_init(ctx, SHA3_256_RATE, 0x06);
}

int
ichor_sha3_256_absorb(ichor_hash_ctx_t *ctx, const uint8_t *data, size_t len)
{
    return sponge_absorb(ctx, data, len);
}

void
ichor_sha3_256_finalize(ichor_hash_ctx_t *ctx, uint8_t out[32])
{
    sponge_squeeze(ctx, out, 32);
}

void
ichor_sha3_256(uint8_t out[32], const uint8_t *data, size_t len)
{
    ichor_hash_ctx_t ctx;
    ichor_sha3_256_init(&ctx);
    ichor_sha3_256_absorb(&ctx, data, len);
...
void
ichor_shake128_init(ichor_hash_ctx_t *ctx)
{
    sponge_init(ctx, SHAKE128_RATE, 0x1f);
}

int
ichor_shake128_absorb(ichor_hash_ctx_t *ctx, const uint8_t *data, size_t len)
{
    return sponge_absorb(ctx, data, len);
}
...
int
ichor_shake128_absorb_u32_le(ichor_hash_ctx_t *ctx, uint32_t v)
{
    uint8_t buf[4];

    pack_u32_le(buf, v);
    return sponge_absorb(ctx, buf, sizeof(buf));
}

int
ichor_shake128_absorb_u64_le(ichor_hash_ctx_t *ctx, uint64_t v)
{
    uint8_t buf[8];
...
void
ichor_shake128_squeeze(ichor_hash_ctx_t *ctx, uint8_t *out, size_t len)
{
...
void
ichor_shake256_init(ichor_hash_ctx_t *ctx)
{
    sponge_init(ctx, SHAKE256_RATE, 0x1f);
}

int
ichor_shake256_absorb(ichor_hash_ctx_t *ctx, const uint8_t *data, size_t len)
{
    return sponge_absorb(ctx, data, len);
}

void
ichor_shake256_squeeze(ichor_hash_ctx_t *ctx, uint8_t *out, size_t len)
{
    sponge_squeeze(ctx, out, len);
}

int
ichor_shake256_absorb_u32_le(ichor_hash_ctx_t *ctx, uint32_t v)
{
    uint8_t buf[4];
...
int
ichor_shake256_absorb_u64_le(ichor_hash_ctx_t *ctx, uint64_t v)
{
    uint8_t buf[8];
...
void
ichor_hash_ctx_clear(ichor_hash_ctx_t *ctx)
{
