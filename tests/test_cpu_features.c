 ...
 *
 * Validates:
 *   - ichor_cpu_features() returns the correct bitmask by cross-checking
 *     against compiler builtins (x86_64) or getauxval (aarch64).
 *   - ichor_cpu_features_override() replaces the cached mask.
 *   - The module is backend-agnostic: no aes.c / field.c / grostl.c
 *     reference to ichor_cpu_features() yet.
 */
...
test_stable(void)
{
    TEST("ichor_cpu_features() stable across two calls");
    uint32_t first = ichor_cpu_features();
    uint32_t second = ichor_cpu_features();
    if (first == second)
        PASS();
    else
...
test_x86_aesni(void)
{
    TEST("x86_64: ICHOR_CPU_AES_NI matches __builtin_cpu_supports(aes)");
    uint32_t mask = ichor_cpu_features();
    int builtin_aes = __builtin_cpu_supports("aes");
    int our_aes = (mask & ICHOR_CPU_AES_NI) != 0;
...
test_x86_clmul(void)
{
    TEST("x86_64: ICHOR_CPU_CLMUL matches __builtin_cpu_supports(pclmul)");
    uint32_t mask = ichor_cpu_features();
    int builtin = __builtin_cpu_supports("pclmul");
    int ours = (mask & ICHOR_CPU_CLMUL) != 0;
    if (ours == (builtin != 0))
        PASS();
    else
...
test_x86_ssse3(void)
{
    TEST("x86_64: ICHOR_CPU_SSSE3 matches __builtin_cpu_supports(ssse3)");
    uint32_t mask = ichor_cpu_features();
    int builtin = __builtin_cpu_supports("ssse3");
    int ours = (mask & ICHOR_CPU_SSSE3) != 0;
    if (ours == (builtin != 0))
        PASS();
    else
...
test_x86_sse41(void)
{
    TEST("x86_64: ICHOR_CPU_SSE41 matches __builtin_cpu_supports(sse4.1)");
    uint32_t mask = ichor_cpu_features();
    int builtin = __builtin_cpu_supports("sse4.1");
    int ours = (mask & ICHOR_CPU_SSE41) != 0;
    if (ours == (builtin != 0))
        PASS();
    else
...
{
    TEST("aarch64: ICHOR_CPU_ARMV8_AES matches getauxval(AT_HWCAP)");
    uint32_t mask = ichor_cpu_features();
    uint64_t hwcap = getauxval(AT_HWCAP);
    int our_aes = (mask & ICHOR_CPU_ARMV8_AES) != 0;
...
{
    TEST("aarch64: ICHOR_CPU_PMULL matches getauxval(AT_HWCAP)");
    uint32_t mask = ichor_cpu_features();
    uint64_t hwcap = getauxval(AT_HWCAP);
    int our_pmull = (mask & ICHOR_CPU_PMULL) != 0;
...
test_override_zero(void)
{
    ...
    uint32_t original = ichor_cpu_features();
    ichor_cpu_features_override(0);
    uint32_t after = ichor_cpu_features();
    /* Restore so later tests in the same process are not affected. */
    ichor_cpu_features_override(original);
    if (after == 0)
        PASS();
    else
...
test_override_restore(void)
{
    ...
    ichor_cpu_features_override(0);
    ichor_cpu_features_override(original);
    uint32_t restored = ichor_cpu_features();
...
test_no_cross_arch_bits(void)
{
    ...
    if (mask & (ICHOR_CPU_ARMV8_AES | ICHOR_CPU_PMULL))
        FAIL("aarch64 bits set on x86_64 host");
    else
        PASS();
#elif defined(__aarch64__)
    /* On aarch64, x86_64 bits must be clear. */
    if (mask & (ICHOR_CPU_AES_NI | ICHOR_CPU_CLMUL | ICHOR_CPU_SSE41 |
                ICHOR_CPU_SSSE3))
