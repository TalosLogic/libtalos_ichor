 ...
 *
 * test_grostl_kat.c - Full NIST KAT validation for the ichor Grøstl
 * primitive.
 *
 * Parses the official Grøstl round-3 submission KAT files shipped in
 * tests/KAT_MCT/ and hashes every byte-aligned record, comparing against
 * the published MD field.  Where test_grostl.c checks a handful of
 * hand-picked boundaries, this sweeps:
 *
...
#ifndef ICHOR_TEST_SOURCE_DIR
#error "ICHOR_TEST_SOURCE_DIR must be defined by the build system"
#endif

#define KAT_DIR ICHOR_TEST_SOURCE_DIR "/tests/KAT_MCT/"
...
run_kat_file(const char *path, size_t digest_bytes)
{
    ...
    while (fgets(line, sizeof(line), f) != NULL) {
        ...
        if (digest_bytes == 32)
            ichor_grostl256(got, msg, msg_bytes);
        else
            ichor_grostl512(got, msg, msg_bytes);
