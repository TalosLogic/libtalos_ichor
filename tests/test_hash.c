...
{
    ichor_hash_ctx_t ctx;
    uint8_t out[32];

    ichor_sha3_256_init(&ctx);
    ichor_sha3_256_absorb(&ctx, (const uint8_t *)"a", 1);
    ichor_sha3_256_absorb(&ctx, (const uint8_t *)"b", 1);
    ichor_sha3_256_absorb(&ctx, (const uint8_t *)"c", 1);
    ichor_sha3_256_finalize(&ctx, out);
...
test_shake128_empty(void)
{
    ...
    ichor_shake128_init(&ctx);
    ichor_shake128_squeeze(&ctx, out, 32);
...
test_shake128_abc(void)
{
    ...
    ichor_shake128_init(&ctx);
    ichor_shake128_absorb(&ctx, msg, 3);
    ichor_shake128_squeeze(&ctx, out, 32);
...
{
    ichor_hash_ctx_t ctx1, ctx2;
    uint8_t out_full[64];
    uint8_t out_parts[64];

    /* One-shot squeeze */
    ichor_shake128_init(&ctx1);
    ichor_shake128_squeeze(&ctx1, out_full, 64);

    /* Two-part squeeze */
    ichor_shake128_init(&ctx2);
    ichor_shake128_squeeze(&ctx2, out_parts, 32);
    ichor_shake128_squeeze(&ctx2, out_parts + 32, 32);
...
test_shake256_empty(void)
{
    ...
    ichor_shake256_init(&ctx);
    ichor_shake256_squeeze(&ctx, out, 32);
...
test_shake256_abc(void)
{
    ...
    ichor_shake256_init(&ctx);
    ichor_shake256_absorb(&ctx, msg, 3);
    ichor_shake256_squeeze(&ctx, out, 32);
...
test_shake256_long_squeeze(void)
{
    ...
    ichor_shake256_init(&ctx);
    ichor_shake256_squeeze(&ctx, out, 512);
...
test_shake256_byte_squeeze(void)
{
    ...
    ichor_shake256_init(&ctx1);
    ichor_shake256_absorb(&ctx1, (const uint8_t *)"test", 4);
    ichor_shake256_squeeze(&ctx1, bulk, 256);

    ichor_shake256_init(&ctx2);
    ichor_shake256_absorb(&ctx2, (const uint8_t *)"test", 4);
    for (int i = 0; i < 256; i++)
        ichor_shake256_squeeze(&ctx2, byte_by_byte + i, 1);
...
test_sha3_256_exact_block(void)
{
    ...
    /* One-shot */
    ichor_sha3_256(out1, msg, 136);

    /* Incremental: two chunks */
    ichor_hash_ctx_t ctx;
    ichor_sha3_256_init(&ctx);
    ichor_sha3_256_absorb(&ctx, msg, 100);
    ichor_sha3_256_absorb(&ctx, msg + 100, 36);
    ichor_sha3_256_finalize(&ctx, out2);
...
test_shake128_200_a3(void)
{
    ...
    ichor_shake128_init(&ctx);
    ichor_shake128_absorb(&ctx, msg, 200);
    ichor_shake128_squeeze(&ctx, out, 16);
...
test_shake256_200_a3(void)
{
    ...
    ichor_shake256_init(&ctx);
    ichor_shake256_absorb(&ctx, msg, 200);
    ichor_shake256_squeeze(&ctx, out, 16);
...
/*
 * Test 16: absorb after squeeze/finalize is rejected.
 *
 * The contract (hash.h) forbids absorbing once squeezing has begun; the
 * absorb interface must return ICHOR_HASH_ERR_FINALIZED and leave the output
 * unchanged rather than silently corrupting it.
 */
static void
test_absorb_after_squeeze_rejected(void)
{
    ichor_hash_ctx_t ctx;
    uint8_t out[32], out_ref[32];
    const uint8_t msg[] = {0x61, 0x62, 0x63};

    /* SHAKE-128: squeeze, then attempt a post-squeeze absorb. */
    ichor_shake128_init(&ctx);
    check("SHAKE128 absorb pre-squeeze returns 0",
          ichor_shake128_absorb(&ctx, msg, 3) == 0);
    ichor_shake128_squeeze(&ctx, out_ref, 32);
    check("SHAKE128 absorb after squeeze returns error",
          ichor_shake128_absorb(&ctx, msg, 3) == ICHOR_HASH_ERR_FINALIZED);

    /* SHA3-256: finalize, then attempt a post-finalize absorb. */
    ichor_hash_ctx_t sctx;
    uint8_t digest[32];
    ichor_sha3_256_init(&sctx);
    ichor_sha3_256_absorb(&sctx, msg, 3);
    ichor_sha3_256_finalize(&sctx, digest);
    check("SHA3-256 absorb after finalize returns error",
          ichor_sha3_256_absorb(&sctx, msg, 3) == ICHOR_HASH_ERR_FINALIZED);
    check("SHAKE128 u32_le absorb after squeeze returns error",
          ichor_shake128_absorb_u32_le(&ctx, 0) == ICHOR_HASH_ERR_FINALIZED);

    /* The rejected absorb must not have perturbed the squeezing state:
     * re-running the same transcript yields the same output. */
    ichor_shake128_init(&ctx);
    ichor_shake128_absorb(&ctx, msg, 3);
    ichor_shake128_squeeze(&ctx, out, 32);
    check("SHAKE128 output unchanged by rejected absorb",
          memcmp(out, out_ref, 32) == 0);
}
...
main(void)
{
    ...
    /* Misuse contract */
    test_absorb_after_squeeze_rejected();
