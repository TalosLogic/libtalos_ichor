 ...
 *
 * test_grostl_mct.c - NIST Monte Carlo Test for the ichor Grøstl
 * primitive.
 *
 ...
 * in tests/KAT_MCT/MonteCarlo_{256,512}.txt and confirmed against
...
mct_step(uint8_t msg[MCT_MSG_BYTES], uint8_t *md, size_t digest_bytes)
{
    ...
    if (digest_bytes == 32)
        ichor_grostl256(md, msg, MCT_MSG_BYTES);
    else
        ichor_grostl512(md, msg, MCT_MSG_BYTES);
