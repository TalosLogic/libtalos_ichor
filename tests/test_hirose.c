 ...
 *
 * Ported from libtalos_voleith (core/hirose -> ichor/hirose); the
 * primitive and these tests are identical apart from the ichor_ prefix.
 *
 * Tests:
 *   1: Output matches the spec formula computed independently via
 *      ichor_aes_encrypt (in-test reference).
 ...
 *      publications or in the original Hirose FSE 2006 paper (which is
 *      a theoretical security analysis without numerical vectors), so
 *      this is the strongest external grounding available.
 */
...
/*
 * In-test reference: directly apply the spec equations using the ichor
 * AES API.  Independent of ichor_hirose_iteration so it actually
                     ...
                     uint8_t G_out[16], uint8_t H_out[16])
{
    ...
    ichor_aes_encrypt(&ctx, E_G, G);
    for (i = 0; i < 16; i++)
        G_out[i] = E_G[i] ^ G[i];

    for (i = 0; i < 16; i++)
        Gxc[i] = G[i] ^ c_const[i];
    ichor_aes_encrypt(&ctx, E_Gxc, Gxc);
    for (i = 0; i < 16; i++)
...
static const uint8_t M_IN[16] = {0xde, 0xad, 0xbe, 0xef, 0xca, 0xfe,
                                 ...
                                 0x89, 0xab, 0xcd, 0xef};
static const uint8_t C_IN[16] = {'I', 'C', 'H', 'O', 'R', '-', 'H', 'i',
                                 'r', 'o', 's', 'e', '-', 'c', '0', '1'};
...
test_matches_reference(void)
{
    ...
    ichor_hirose_iteration(G_IN, H_IN, M_IN, C_IN, G_out, H_out);
...
test_in_place_safe(void)
{
    ...
    ichor_hirose_iteration(G_IN, H_IN, M_IN, C_IN, G_ref, H_ref);

    /* G_out aliases G; H_out aliases H. */
    memcpy(G_alias, G_IN, 16);
    memcpy(H_alias, H_IN, 16);
    ichor_hirose_iteration(G_alias, H_alias, M_IN, C_IN, G_alias, H_alias);
...
test_c_is_consumed(void)
{
    ...
    ichor_hirose_iteration(G_IN, H_IN, M_IN, C_IN, G_a, H_a);

    memcpy(c_alt, C_IN, 16);
    c_alt[0] ^= 0x01;
    ichor_hirose_iteration(G_IN, H_IN, M_IN, c_alt, G_b, H_b);
...
test_m_changes_key(void)
{
    ...
    M_alt[7] ^= 0x80;
    ichor_hirose_iteration(G_IN, H_IN, M_alt, C_IN, G_b, H_b);
...
test_inputs_differentiate(void)
{
    uint8_t G_a[16], H_a[16];
    uint8_t G_b[16], H_b[16];
    uint8_t G_alt[16], H_alt[16];

    ichor_hirose_iteration(G_IN, H_IN, M_IN, C_IN, G_a, H_a);

    memcpy(G_alt, G_IN, 16);
    G_alt[0] ^= 0x01;
    ichor_hirose_iteration(G_alt, H_IN, M_IN, C_IN, G_b, H_b);
    ...
    H_alt[15] ^= 0x40;
    ichor_hirose_iteration(G_IN, H_alt, M_IN, C_IN, G_b, H_b);
 ...
 * without computing any AES themselves.  Matching this value confirms
 * three things at once: the H||M -> K assembly order, the "G as
 * plaintext" plaintext assignment, and the "AES_K(G) XOR G" output
 * formula.  H_next is still checked against the in-test reference
 * because FIPS 197 publishes only one (key, pt, ct) triple per key and
 * so cannot directly pin the second encryption AES_K(G XOR c).
 */
static void
test_g_next_matches_fips197_c3(void)
{
     ...
     * test is independent of any inode/leaf constants a consumer
     * chooses for its framing.  This c is local to this test only.
     */
    ...
    ichor_hirose_iteration(G_in, H_in, M_in, c_local, G_out, H_out);

    /*
     * Load-bearing assertion: G_out matches the value derived by hand
     * from FIPS 197 + the spec equation.
     */
