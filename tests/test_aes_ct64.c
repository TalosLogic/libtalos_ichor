 ...
 * differential vs ichor_aes) live in this file.
 *
 ...
 *   - Differential vs ichor_aes over random inputs.
 *
 ...
 * the ichor_aes backend.  A pure permutation bug in the
...
test_packing_sentinel(void)
{
    ...
    ichor_aes_key_expand(&ctx_ref, key, 128);

    aes_ct64_encrypt(&ctx_bs, ct_bs, pt);
    ichor_aes_encrypt(&ctx_ref, ct_ref, pt);

    aes_ct64_ctx_clear(&ctx_bs);
    ichor_aes_ctx_clear(&ctx_ref);
 ...
 * with byte 0 walking through 0..255, compare against ichor_aes
...
/* ================================================================
 * Differential vs ichor_aes_* over random inputs.  The KAT
...
differential_one(int key_bits, int trials, const char *name)
{
    ...
    for (int t = 0; t < trials && !fail; t++) {
        ...
        ichor_aes_key_expand(&ctx_ref, key, key_bits);
...
main(void)
{
    ...
    printf("==============\n");
    printf("ichor_aes active backend: %s\n", ichor_aes_backend_name());

    /* Backend-specific tests. */
    printf("\n  Backend-specific\n");
    test_packing_sentinel();
    test_sbox_through_full_encrypt();
    differential_one(128, 64, "AES-128 differential vs ichor_aes (64 trials)");
    differential_one(192, 64, "AES-192 differential vs ichor_aes (64 trials)");
    differential_one(256, 64, "AES-256 differential vs ichor_aes (64 trials)");
    test_x4_consistency();
