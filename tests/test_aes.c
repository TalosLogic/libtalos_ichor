 ...
 *
 * test_aes.c - Tests for the ichor_aes public API.
 *
 ...
 * once with ICHOR_FORCE_BACKEND=aes:bitsliced -- to cover both paths.
 */
...
static int
ichor_aes_kat_adapter(int key_bits, const uint8_t *key, uint8_t out[16],
                      const uint8_t in[16])
{
    ichor_aes_ctx_t ctx;
    if (ichor_aes_key_expand(&ctx, key, key_bits) != 0)
        return -1;
...
/* ================================================================
 * ichor_aes_encrypt_x4 vs four serial single-block encrypts.
 * ================================================================ */

static void
test_aes128_encrypt_x4(void)
{
    TEST("AES-128 ichor_aes_encrypt_x4 vs four serial encrypts");
    ...
    ichor_aes_key_expand(&ctx, key, 128);
    ichor_aes_encrypt_x4(&ctx, out_x4, in);
    for (int b = 0; b < 4; b++)
        ichor_aes_encrypt(&ctx, out_serial + 16 * b, in + 16 * b);
    ...
    uint8_t buf[64];
    memcpy(buf, in, 64);
    ichor_aes_encrypt_x4(&ctx, buf, buf);
...
test_invalid_key_size(void)
{
    ...
    if (ichor_aes_key_expand(&ctx, key, 64) == -1 &&
        ichor_aes_key_expand(&ctx, key, 0) == -1 &&
        ichor_aes_key_expand(&ctx, key, 512) == -1) {
        PASS();
    } else {
...
run_all_tests(void)
{
    ...
    if (be == ICHOR_AES_BACKEND_AESNI || be == ICHOR_AES_BACKEND_ARMV8) {
    ...
    aes_kat_run_all("ichor_aes", ichor_aes_kat_adapter, &tests_run,
                    &tests_passed);
}
