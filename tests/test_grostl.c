 ...
 *
 * test_grostl.c - Correctness tests for the ichor Grøstl primitive.
 *
 * Validates Grøstl-256 and Grøstl-512 against a hand-picked subset of the
 * NIST round-3 KAT vectors.  Vectors are embedded so the test needs no
 * external files.  The full ShortMsg/LongMsg/Monte-Carlo sweep remains in
 * libtalos_voleith (against the ichor-backed shim); this test is ichor's
 * self-contained parity oracle.
 *
 * Runs against whatever Grøstl backend is active.  CTest registers this
 * binary twice via ichor_add_test -- once with the hardware backend and
 * once with ICHOR_FORCE_BACKEND=aes:bitsliced, which forces the bitsliced
 * SubBytes path (Grøstl dispatches on the same AES feature bits) -- so both
 * paths are covered on every host.
 *
 * Vectors covered:
 *   - Grøstl-256 and Grøstl-512: empty input, 3-byte, 64-byte, 128-byte.
 *   - Incremental absorb matches one-shot API (exercises buffer management).
 *   - Fixed-input single-compression node-hash contract.
 */
...
/* ================================================================
 * NIST KAT vectors hand-extracted from the round-3 submission's
 * ShortMsgKAT_{256,512}.txt.
 * ================================================================ */
...
test_grostl256_3byte(void)
{
    ...
    uint8_t md[32];
    ichor_grostl256(md, KAT256_3BYTE_MSG, sizeof(KAT256_3BYTE_MSG));
...
test_grostl512_3byte(void)
{
    ...
    uint8_t md[64];
    ichor_grostl512(md, KAT256_3BYTE_MSG, sizeof(KAT256_3BYTE_MSG));
...
test_incremental_256(void)
{
    ...
    ichor_grostl256(md_oneshot, msg, 128);

    ichor_grostl_ctx_t ctx;
    ichor_grostl256_init(&ctx);
    for (int i = 0; i < 128; i++)
        ichor_grostl_absorb(&ctx, msg + i, 1);
    ichor_grostl_finalize(&ctx, md_inc);
...
test_incremental_512(void)
{
    ...
    ichor_grostl512(md_oneshot, msg, 128);

    ichor_grostl_ctx_t ctx;
    ichor_grostl512_init(&ctx);
    ichor_grostl_absorb(&ctx, msg + 0, 1);
    ichor_grostl_absorb(&ctx, msg + 1, 63);
    ichor_grostl_absorb(&ctx, msg + 64, 32);
    ichor_grostl_absorb(&ctx, msg + 96, 32);
    ichor_grostl_finalize(&ctx, md_inc);
    ichor_grostl_clear(&ctx);
 ...
 * in the consumer by the circuit-vs-oracle test.  Here we check the
 * contract: determinism, NULL rejection, that distinct chaining values
 * (iv) yield distinct outputs (domain separation is carried by iv), and
...
test_node_256_contract(void)
{
    ...
    if (ichor_grostl256_compress_node(iv_a, block, out_a) != 0 ||
        ichor_grostl256_compress_node(iv_a, block, out_a2) != 0 ||
        ichor_grostl256_compress_node(iv_b, block, out_b) != 0 ||
        ichor_grostl256_compress_node(iv_a, block2, out_blk2) != 0) {
        FAIL("unexpected nonzero return");
        return;
    }
    if (ichor_grostl256_compress_node(NULL, block, out_a) >= 0) {
...
test_node_512_contract(void)
{
    ...
    if (ichor_grostl512_compress_node(iv_a, block, out_a) != 0 ||
        ichor_grostl512_compress_node(iv_a, block, out_a2) != 0 ||
        ichor_grostl512_compress_node(iv_b, block, out_b) != 0 ||
        ichor_grostl512_compress_node(iv_a, block2, out_blk2) != 0) {
        FAIL("unexpected nonzero return");
        return;
    }
    if (ichor_grostl512_compress_node(iv_a, NULL, out_a) >= 0) {
...
/*
 * Fixed-input streaming (init_iv + absorb + finalize_fixed) must agree with
 * the single-compression node hash, which the contract tests above already
 * anchor.  For a one-block message the finalize adds no padding block, so the
 * streaming digest is exactly one compression of (iv, block) followed by the
 * output transform: identical to compress_node.  The empty message pads one
 * zero block, i.e. compress_node(iv, zeros).
 */
static void
test_fixed_256_matches_node(void)
{
    uint8_t iv[64], iv2[64], block[64], zero[64];
    uint8_t stream[32], node[32], stream_empty[32], node_empty[32], stream2[32];
    ichor_grostl_ctx_t ctx;
    int i;

    TEST("grostl256 init_iv+finalize_fixed == compress_node");

    for (i = 0; i < 64; i++) {
        iv[i] = (uint8_t)(i + 1);
        iv2[i] = (uint8_t)(i ^ 0x3c);
        block[i] = (uint8_t)(0xa0 - i);
        zero[i] = 0;
    }

    /* Single full block. */
    ichor_grostl256_init_iv(&ctx, iv);
    ichor_grostl_absorb(&ctx, block, 64);
    ichor_grostl_finalize_fixed(&ctx, stream);
    ichor_grostl_clear(&ctx);
    if (ichor_grostl256_compress_node(iv, block, node) != 0) {
        FAIL("compress_node returned nonzero");
        return;
    }
    if (memcmp(stream, node, 32) != 0) {
        FAIL("single-block stream != node");
        return;
    }

    /* Empty message pads one zero block. */
    ichor_grostl256_init_iv(&ctx, iv);
    ichor_grostl_finalize_fixed(&ctx, stream_empty);
    ichor_grostl_clear(&ctx);
    if (ichor_grostl256_compress_node(iv, zero, node_empty) != 0) {
        FAIL("compress_node(zero) returned nonzero");
        return;
    }
    if (memcmp(stream_empty, node_empty, 32) != 0) {
        FAIL("empty stream != node(zeros)");
        return;
    }

    /* IV carries domain separation: a different IV must change the digest. */
    ichor_grostl256_init_iv(&ctx, iv2);
    ichor_grostl_absorb(&ctx, block, 64);
    ichor_grostl_finalize_fixed(&ctx, stream2);
    ichor_grostl_clear(&ctx);
    if (memcmp(stream, stream2, 32) == 0) {
        FAIL("distinct IV gave same digest");
        return;
    }
    PASS();
}
...
static void
test_fixed_512_matches_node(void)
{
    uint8_t iv[128], iv2[128], block[128], zero[128];
    uint8_t stream[64], node[64], stream_empty[64], node_empty[64], stream2[64];
    ichor_grostl_ctx_t ctx;
    int i;

    TEST("grostl512 init_iv+finalize_fixed == compress_node");
    ...
    ichor_grostl512_init_iv(&ctx, iv);
    ichor_grostl_absorb(&ctx, block, 128);
    ichor_grostl_finalize_fixed(&ctx, stream);
    ichor_grostl_clear(&ctx);
    if (ichor_grostl512_compress_node(iv, block, node) != 0) {
        FAIL("compress_node returned nonzero");
        return;
    }
    if (memcmp(stream, node, 64) != 0) {
        FAIL("single-block stream != node");
        return;
    }
    ...
    ichor_grostl_finalize_fixed(&ctx, stream_empty);
    ichor_grostl_clear(&ctx);
    if (ichor_grostl512_compress_node(iv, zero, node_empty) != 0) {
        FAIL("compress_node(zero) returned nonzero");
        return;
    }
    if (memcmp(stream_empty, node_empty, 64) != 0) {
        FAIL("empty stream != node(zeros)");
        return;
    }

    ichor_grostl512_init_iv(&ctx, iv2);
    ichor_grostl_absorb(&ctx, block, 128);
    ichor_grostl_finalize_fixed(&ctx, stream2);
    ichor_grostl_clear(&ctx);
    if (memcmp(stream, stream2, 64) == 0) {
        FAIL("distinct IV gave same digest");
        return;
    }
    PASS();
...
run_dispatched_tests(void)
{
    ...
    printf("\n    Fixed-input streaming (init_iv + finalize_fixed)\n");
    test_fixed_256_matches_node();
    test_fixed_512_matches_node();
}
